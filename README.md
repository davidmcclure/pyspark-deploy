...
Of course, Docker solves all of these problems! But, deploying a completely Dockerized Spark cluster also takes a bit of work. But, if we assume that the entire Spark + Python + application environment is contained in a Docker image, the actual cluster deployment can identical for almost all projects.
